...
def obter_llm() -> ChatOpenAI:
    return ChatOpenAI(
        ...
        openai_api_key='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'
    )

# Função para criar o LLMChain com memória de conversação
def obter_llm_chain():
    template = """
    Você é um assistente especializado em ajudar os clientes da Web Continental.
...
def carregar_produtos():
    caminho_arquivo = "D:/DataExperience/Zoe/zoe_webcontinental/produtos/ar_condiconado.csv" 
...
def processar_entrada(df, prompt):
    ...
    elif intencao == "listar_todos_modelos":
...
def inicializar_estado_sessao():
    if MENSAGENS not in st.session_state:
        st.session_state[MENSAGENS] = [
            Mensagem(
                ator=ASSISTENTE,
                conteudo="Olá, sou seu assistente da Web Continental. Em que posso ajudar?"
            )
        ]
